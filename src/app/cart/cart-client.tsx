...
export default function CartPageClient() {
  ...
  if (loading) return <div className="section container-wide text-slate-400">Loading cart...</div>;
        ...
        {lines.length === 0 ? (
          ...
          <>
              ...
              {lines.map((line) => (
                <div key={`${line.type}-${line.slug}`} className="card p-4 flex gap-4 items-center">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {!line.purchasable && (
                      <p className="text-xs text-yellow-400 mt-1">Requires quote — remove to checkout other items</p>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-white w-6 text-center">{line.quantity}</span>
                    <button type="button" className="btn-outline px-2 py-1 text-sm" onClick={() => updateQty(line.type, line.slug, line.quantity + 1)}>+</button>
                  </div>
                  <div className="text-white font-medium">${(line.priceUsd * line.quantity).toLocaleString()}</div>
            ...
            <div className="card p-6 flex justify-between items-center">
              <span className="text-lg font-bold text-white">Total: ${total.toLocaleString()}</span>
